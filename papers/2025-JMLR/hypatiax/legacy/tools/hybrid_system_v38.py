...
# ──────────────────────────────────────────────────────────────────
"""
HypatiaX Hybrid Discovery System v3.8 - FIXED AUTO-CONFIGURATION
Complete integration with v8 symbolic engine:

CRITICAL FIXES IN v3.8:
- Disable PhysicsAware fallback by default (it was making things worse!)
- Increase SymbolicEngine retries (3 -> 5)
- Pass auto-config directly to SymbolicEngine (no override)
- Simpler quality checking
- Better logging
- Integration with symbolic_engine_v8.py
- FIXED: _check_expression_quality method now properly defined
"""
...
class HybridDiscoverySystem:
    """
    Hybrid discovery system v3.8 with FIXED auto-configuration.

    Key changes:
    - PhysicsAware fallback DISABLED by default (it ignores auto-config!)
    - More SymbolicEngine retries (5 instead of 3)
    - Direct auto-config passthrough (no mutation)
    - Simpler validation
    - FIXED: Proper _check_expression_quality implementation
    """
    ...
    def __init__(
        ...
        enable_physics_fallback: bool = False,  # CHANGED: Default FALSE
        ...
        logger.info(f"=" * 70)
        logger.info(f"HybridDiscoverySystem v3.8 - FIXED AUTO-CONFIGURATION")
        logger.info(f"=" * 70)
        ...
        # Configure symbolic engine with v8 defaults
        symbolic_config = discovery_config or DiscoveryConfig(
            niterations=100,  # Higher default
        ...
        # Initialize LLM providers (optional, for backward compatibility)
        ...
        logger.info("[OK] HybridDiscoverySystem v3.8 initialized\n")
    ...
    def _create_optimized_physics_regressor(self) -> PhysicsAwareRegressor:
        """Create optimized physics-aware regressor."""
    ...
    def _check_expression_quality(self, expression: str, r2: float) -> Dict[str, Any]:
        """
        Simple quality check for discovered expressions.

        Args:
            expression: The discovered symbolic expression
            r2: R² score of the expression

        Returns:
            dict: Quality assessment with 'is_overfit', 'complexity', 'warnings'
        """
        ...
        if complexity > self.complexity_penalty_threshold and r2 < 0.999:
            is_overfit = True
            warnings.append(f"High complexity ({complexity}) but R2={r2:.4f}")

        # Check for suspicious constants (possible overfitting indicators)
        constants = re.findall(r"\d+\.\d+", expression)
        if len(constants) > 5:
            warnings.append(f"Many constants detected ({len(constants)})")

        # Check for very small or very large constants
    ...
    def _discover_with_retry(
        ...
        """
        Discover with retry - v3.8 simplified version.

        NO PhysicsAware fallback by default!
        Just retry SymbolicEngine with different seeds.
        """
        best_result = None
        best_r2 = -np.inf

        # Try SymbolicEngine multiple times
        for attempt in range(self.max_retries):
            try:
                ...
                # Let SymbolicEngine handle auto-config internally
        ...
        # Optional PhysicsAware fallback
        ...
        if best_result:
            ...
            raise ValueError(f"All discovery attempts failed")
    ...
    def discover_validate_interpret(
        ...
        """
        Complete discovery workflow v3.8.
        """
        print(f"\n{'=' * 70}")
        print(f"DISCOVERY WORKFLOW v3.8")
        print(f"{'=' * 70}")
        ...
        try:
            ...
            # Show auto-config info if available
        ...
        if validation_result.get("validation_exception"):
            print(f"   [WARNING] Validation had errors (likely unit system)")
    ...
    def save_results(self, filename: str = None):
        ...
        if filename is None:
            ...
            filename = f"discovery_results_v38_{timestamp}.json"

        # Convert deque to list and make JSON serializable
        results_list = []
...
if __name__ == "__main__":
    print("=" * 80)
    print("HYBRID SYSTEM v3.8 - QUICK TEST")
    print("=" * 80)
    ...
    system = HybridDiscoverySystem(
        ...
        enable_physics_fallback=False,  # No fallback!
        max_retries=5,
    )
